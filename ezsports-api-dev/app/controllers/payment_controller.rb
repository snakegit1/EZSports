				...
				amount: amount, 
					...
					if cc_id == nil # if there is no cc_id, they have not been created as a Braintree customer
						...
						if customer_result.success?
							...
							card = League.find_or_initialize_by(id: league_id, user_id: user.id)
							card.cc_id = customer_result.customer.id
							card.cc_number = params[:cc_number]
							card.cvv_number = params[:cc_cvv]
							card.exp_month = params[:cc_month]
							card.exp_year = params[:cc_year]
							card.last_digit = customer_result.customer.credit_cards.last.last_4
							card.cc_token = customer_result.customer.credit_cards.last.token
						...
						customer = Braintree::Customer.find(cc_id)

						if league.cc_token.present?
							customer_cc_result = Braintree::PaymentMethod.find(league.cc_token).update(payment_method_nonce: payment_method_nonce)
							customer_cc = customer_cc_result.credit_card
						else
							customer_cc_result = Braintree::PaymentMethod.create(customer_id: cc_id, payment_method_nonce: payment_method_nonce)
							customer_cc = customer_cc_result.payment_method
						end

						if customer.present?
							p 'Braintree created customer successfully'
							card = League.find_or_initialize_by(id: league_id, user_id: user.id)
							card.cc_id = customer.id
							card.cc_number = params[:cc_number]
							card.cvv_number = params[:cc_cvv]
							card.exp_month = params[:cc_month]
							card.exp_year = params[:cc_year]
							card.last_digit = customer_cc.last_4
							card.cc_token = customer_cc.token
