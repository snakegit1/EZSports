...
class UsersController < BaseController
	...
	def signup
		begin
			...
			result = Braintree::CreditCard.create(
				...
				:expiration_date => "#{params[:cc][:month]}/#{params[:cc][:year]}",
				:cvv => params[:cc][:cvv]
			)
			if result.success?
				if user.credit_card
					user.credit_card.update(
						last_4: result.credit_card.last_4,
						token: result.credit_card.token
					)
				else
					user.credit_card = CreditCard.create(
						last_4: result.credit_card.last_4,
						token: result.credit_card.token,
						customer_id: result.credit_card.customer_id
					)
				end
			else
				puts result.errors
	...
	def update_cc
		begin
			...
			# Create Braintree Customer for user if not exist
			...
			render json: {last_4: user.credit_card.last_4}
		rescue => e
			HandleError(e);
			return nil;
		end
	...
	def delete_cc
		...
		begin
			last_4 = params[:cc_last_4]
			credit_card = CreditCard.find_by(last_4: last_4, customer_id: user.customer_id)
			if credit_card
				begin
					Braintree::CreditCard.find(credit_card.token).delete
					credit_card.delete
	...
	def load_admin_login_data(user)
		...
		if user.credit_card
			cc_last_4 = user.credit_card.last_4
		else
			cc_last_4 = nil
		end
